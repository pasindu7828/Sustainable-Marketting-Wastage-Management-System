...
const UserOrderReport = () => {
  ...
  const handleDownload = async () => {
    ...
    doc.setFontSize(18);
    doc.text('User Order Report', 40, 40);
    // Summary cards
    const cards = document.getElementById('summary-cards');
    if (cards) {
      const cardsCanvas = await html2canvas(cards);
      doc.addImage(cardsCanvas.toDataURL('image/png'), 'PNG', 40, 50, 520, 80);
    }
    // Table
    const table = reportRef.current;
    const tableCanvas = await html2canvas(table);
    doc.addImage(tableCanvas.toDataURL('image/png'), 'PNG', 40, 140, 520, 220);
    // Charts
    ...
    doc.save('user_order_report.pdf');
  };
